	...
	Y.Moodle.qtype_drawing.form = {
			...
			return;
				var frame = document.getElementById("qtype_drawing_editor_"+questionID);
					svgCanvas = new embedded_svg_edit(frame);console.error("0e..");
			  	svgCanvas.clear();
				var iframe = document.getElementById("qtype_drawing_editor_"+questionID);
				...
				//		var base64img = btoa(background);
						background = 'data:image/svg+xml,' + decoded;
console.error(background);

					//	svgCanvas.setBackground("",vg);
				//		console.error(vg);
					}
...
var loadimg = function(){
	...
	backgroundimg.onload = function () {
		console.error("Image - attempt loaded");
	//	$('#canvas_width').val(width);
//		$('#canvas_height').val(height);
...
svgCanvas.updateCanvas(width, height);
		...
		svgCanvas.setResolution(width, height);
		svgCanvas.updateCanvas(width, height);
		document.getElementById("q1").appendChild(backgroundimg);
	}.bind(this);
...
*/
					...
					backgroundimg.onerror = function () {
						console.error("background assign error");
					}.bind(this);
					backgroundimg.src = background;
*/
				}
    ...
    type: mimeType
			...
			correctanswer: function(questionID, background, studentanswer, width, height) {
				var studentanswer_b64 = 'data:image/svg+xml;base64,\n'+btoa(studentanswer.replace(/(\r\n|\n|\r)/gm," "));

				document.getElementById('qtype_drawing_drawingwrapper_'+questionID).innerHTML='<img src="'+background+'" style="vertical-align:top;display: inline-block;position: absolute;display: block;top: 0;left: 0;width:100%; height:auto;max-width:'+width+'px; max-height:'+height+'px"><img src="'+studentanswer_b64+'" style="vertical-align:top;display: inline-block;position: absolute;top: 0;left: 0;opacity: 1;filter: alpha(opacity=100);width:100%; height:auto;max-width:'+width+'px; max-height:'+height+'px">';

			},
			blendedanswer: function(questionID, background, teacheranswer, studentanswer, width, height) {
				var studentanswer_b64 = 'data:image/svg+xml;base64,\n'+btoa(studentanswer.replace(/(\r\n|\n|\r)/gm," "));
				var teacheranswer_b64 = 'data:image/svg+xml;base64,\n'+btoa(teacheranswer.replace(/(\r\n|\n|\r)/gm," "));

				document.getElementById('qtype_drawing_drawingwrapper_'+questionID).innerHTML='<img src="'+background+'" style="vertical-align:top;display: inline-block;position: absolute;display: block;top: 0;left: 0;width:100%; height:auto;max-width:'+width+'px; max-height:'+height+'px"><img src="'+studentanswer_b64+'" style="vertical-align:top;display: inline-block;position: absolute;top: 0;left: 0;opacity: 1;filter: alpha(opacity=100);width:100%; height:auto;max-width:'+width+'px; max-height:'+height+'px"><img src="'+teacheranswer_b64+'" style="padding-bottom: 60%;vertical-align:top;display: inline-block;position: absolute;top: 0;left: 0;opacity: .15;filter: alpha(opacity=15);width:100%; height:auto;max-width:'+width+'px; max-height:'+height+'px">';
				...
				if (typeof correctAnswer != 'undefined' && correctAnswer != 'undefined') {
					...
					if (typeof questionID != 'undefined') {
						...
						if (questionID == 0) {
							...
							// This is a question edit or "add new" form
							// Check if this is an edit form with a pre-existing (on the server) saved image:
							if (Y.one(SELECTORS.CHOOSEANOTHERFILEBUTTON) != null) {this.emptyCanvasDataURL[questionID] = 0;
								// So if there's a pre-existing background image
								// we'd like to hide the file-picker widget (until further notice... (click by 'choose another background'...)
								this.edit_mode = true;
								Y.one(SELECTORS.FILEPICKERFIELDSET).setStyles({display: 'none'});
							...
							this.create_canvas_new_question(questionID);
						} else {
							this.create_canvas_edit_question(questionID);
						}
					}
...
/*
			...
			var drawingEditor = document.getElementById('qtype_drawing_drawingwrapper_'+questionID);

			drawingEditor.style.display = "block";
			drawingEditor.style.pointerEvents = "auto";
			drawingEditor.style.opacity = "1";
			drawingEditor.style.filter  = 'alpha(opacity=100)'; // IE fallback
			Y.one(SELECTORS.NOBACKGROUNDIMAGESELECTEDYET).setStyles({display: 'none'});

			frame.onload = function () {
				var innerDoc = frame.contentDocument || frame.contentWindow.document;
				if (innerDoc) {
					innerDoc.getElementById('canvas_width').value = backgroundimg.width;
					innerDoc.getElementById('canvas_height').value = backgroundimg.height;
				}
			}
*/
		}.bind(this);
	   ...
	   var image = new Image();
		if (Xtextarea != null) {
			if (applyTextArea == false) {
				...
				if(document.getElementById('pre_existing_background_data')){
						document.getElementById('pre_existing_background_data').value = '';
						document.getElementById('id_qtype_drawing_textarea_id_0').value = '';
				}

				// there is update for the question, so just add as new background to fix scaling..
				if(originalBackGroundURL == 'undefined' || !originalBackGroundURL) {
					originalBackGroundURL = '';
					EditingBackGroundURL = Y.one(SELECTORS.DRAWINGEDITMODEBG).get('value');
					OriginalBackgroundImage.src = EditingBackGroundURL;
				}
				*/
				//alert(originalBackGroundURL);
