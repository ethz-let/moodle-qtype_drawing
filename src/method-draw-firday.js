(function() {
  ...
  if(!window.methodDraw) window.methodDraw = function($) {
    ...
    var laststroke = curConfig.initStroke.width;
    var lastopacity = curConfig.initStroke.opacity;
    ...
    Editor.init = function() {
      ...
      $.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
        ...
        fallback:{
          ...
          'exitfullscreen':'exitfullscreen.png',
          'fill':'fill.png',
          'delete':'delete.png',
      ...
      var setPencilMode = function() {
        var curr = $('.tool_button_current');
        if(curr.length && curr[0].id !== 'tool_fhpath') {
          curr.removeClass('tool_button_current').addClass('tool_button');
          $('#tool_fhpath').addClass('tool_button_current').removeClass('tool_button');
        }
      ...
      var setMathJaxMode = function() {
        var curr = $('.tool_button_current');
        if(curr.length && curr[0].id !== 'tool_mathjax') {
          curr.removeClass('tool_button_current').addClass('tool_button');
          $('#tool_mathjax').addClass('tool_button_current').removeClass('tool_button');
        }
      ...
      var selectedChanged = function(window,elems) {
        ...
        selectedElement = (elems.length == 1 || elems[1] == null ? elems[0] : null);console.error(selectedChanged);
...
//            || elem && elem.tagName == "path" && !multiselected) { // This was added in r1430, but not sure why
      ...
      var zoomChanged = function(window, bbox, autoCenter) {
        var scrbar = 15,
          ...
          // fix by amr to keep hight and width alwayz zero so doesnt jump hundreds of times
          bbox.width = 0;
          bbox.height = 0;
      ...
      var setupFlyouts = function(holders) {
        $.each(holders, function(hold_sel, btn_opts) {
          ...
          shower.on('mousedown touchstart pointerdown',function(evt) {
        //  shower.mousedown(function(evt) {
            evt.preventDefault();
            $('#workarea').one("mousedown touchstart pointerdown", function(){
        ...
        //    },time);
          ...
          }).on('mouseup touchend pointerup',function(evt) {
            evt.preventDefault();
            clearTimeout(timer);
...
var updateContextPanel = function(e) {
  var elem = selectedElement;console.error("update context panel, selectedElement: ",selectedElement);
  ...
  if (elem != null) {console.error('element name: ',elem);
    $("#stroke_panel").show();
...
console.error(selectedElement);
        ...
        if (elem != null) {
          ...
          //  if(el_name == 'circle' || el_name == 'line'  || el_name == 'rect' || el_name == 'ellipse') {
          //    $('#drawingpreset_panel').show();
          //  }
            ...
            if(el_name == 'text') {
              ...
              $('#text_panel').css("display", "inline");
            //  $('#textpreset_panel').css("display", "inline");
              $('#tool_italic').toggleClass('active', svgCanvas.getItalic())
              $('#tool_bold').toggleClass('active', svgCanvas.getBold())
              $('#font_family').val(font_family);
              $('#font_size').val(elem.getAttribute("font-size"));
    ...
    return;*/
      ...
      // - adds the tool_button_current class to the button passed in
       toolButtonClick = function(button, noHiding) {
        ...
        $('#drawingpreset_panel').show();
        // now reset last opacity and stroke! Amr
        if(button != '#tool_text') {
          svgCanvas.clearSelection(true);
          ResetStrokeOpocityColor(curConfig.initStroke.width, curConfig.initStroke.opacity, '', 'stroke');
        } else {
          svgCanvas.clearSelection(true);
          ResetStrokeOpocityColor(0, 1, '', 'stroke');
        }
        return true;
      };
      ...
      */
      ...
      var clickSelect = function() {
        if (toolButtonClick('#tool_select')) {svgCanvas.clearSelection(true);
           updateContextPanel();
          svgCanvas.setMode('select');

          //svgCanvas.setMode('select');
          //svgCanvas.clearSelection(true);
        //  updateContextPanel();console.error("select");
        //  resetSelectedPresetsCSS();
          /* amr 2019
          svgCanvas.clearSelection(true);
          // Amr hide canvas panel instead of show
          $("#canvas_panel").hide();
          $("#text_panel").hide();
          $("#stroke_panel").hide();
          $('#selected_panel').hide();
          $('#path_panel').hide();
          $('#drawingpreset_panel').hide();
          $('#textpreset_panel').hide();
          fillOrStrokeColor = 'stroke';
          ResetStrokeOpocityColor(curConfig.initStroke.width, curConfig.initStroke.opacity, '', fillOrStrokeColor);
          */
        }
      };

      clickFHPath = function() {
       if (toolButtonClick('#tool_fhpath')) {
          console.error("path tool clicked.");
          svgCanvas.setMode('fhpath');
      ...
      // Added by Amr.
       resetSelectedPresetsCSS = function() {
        ...
        /*  $('.qtype_drawing_size_pen div').removeClass("qtype_drawing_active_selection");
          ...
          // hide unnecessary panels (no selection while drawing).
          $("#canvas_panel").hide();
          $("#rect_panel").hide();
          $("#path_panel").hide();
          $("#image_panel").hide();
...
*/
      ...
      // Added by Amr.
      var ResetStrokeOpocityColor = function(stroke, opacity, color, fillOrStrokeColor){
        opacity_new = opacity * 100;
        $('#stroke_width').val(stroke);
        $('#group_opacity').val(opacity_new);
        svgCanvas.setStrokeWidth(stroke);
        svgCanvas.setOpacity(opacity);
        // color now to initial
        if (color && color != '') {
            if (color == 'none'){
              var newcolor = 'none';
            } else {
            ...
            var picker = fillOrStrokeColor;
            var color = newcolor;
            var paint = null;
            var noUndo = true;
            paint = new $.jGraduate.Paint({alpha: 100, solidColor: color.substr(1)});
            Editor.paintBox[picker].setPaint(paint);
            svgCanvas.setColor(picker, color, noUndo);
            if (color != 'none' && svgCanvas.getStrokeOpacity() != 1) {
      ...
      var clickPEN1 = function() {
          $('#stroke_width').val(curConfig.initStroke.width);
          $('#group_opacity').val(curConfig.initStroke.opacity * 100);
          svgCanvas.setStrokeWidth(curConfig.initStroke.width); //curConfig.initStroke.width
          svgCanvas.setOpacity(curConfig.initStroke.opacity);
          /*
                    svgCanvas.setStrokeWidth(curConfig.initStroke.width); //curConfig.initStroke.width
                    svgCanvas.setOpacity(curConfig.initStroke.opacity);
          */
                    // now reset color to black
                    var picker = "stroke";
                    var color = '#'+curConfig.initStroke.color;
                    var paint = null;
                    var noUndo = true;
                    paint = new $.jGraduate.Paint({alpha: 100, solidColor: color.substr(1)});
                    Editor.paintBox[picker].setPaint(paint);
                    ...
                    // now reset color to black
                    var picker = "fill";
                    var color = '#'+curConfig.initFill.color; //curConfig.initStroke.color
                    var paint = null;
                    var noUndo = true;
                    paint = new $.jGraduate.Paint({alpha: 100, solidColor: color.substr(1)});
                    Editor.paintBox[picker].setPaint(paint);
          ...
          $('#tool_PEN1 div').addClass("qtype_drawing_active_selection");
          $('#tool_PEN2 div').removeClass("qtype_drawing_active_selection");
          $('#tool_PEN3 div').removeClass("qtype_drawing_active_selection");
          $('#tool_PEN4 div').removeClass("qtype_drawing_active_selection");
        //  svgCanvas.setMode('fhpath');
      //  }
      };
      var clickPEN2 = function() {
      //  if (toolButtonClick('#tool_PEN2')) {
          svgCanvas.setStrokeWidth(10);
          svgCanvas.setOpacity(1);
          $('#stroke_width').val(10).trigger("change");
          $('#group_opacity').val(100).trigger("change");
/*
          svgCanvas.setStrokeWidth(20);
          svgCanvas.setOpacity(0.3);
*/
          var picker = "stroke";
          var color = '#000000';
          var paint = null;
          ...
          $('#tool_PEN1 div').removeClass("qtype_drawing_active_selection");
          $('#tool_PEN2 div').addClass("qtype_drawing_active_selection");
          $('#tool_PEN3 div').removeClass("qtype_drawing_active_selection");
          $('#tool_PEN4 div').removeClass("qtype_drawing_active_selection");
      //  }
      };
      var clickPEN3 = function() {
      //  if (toolButtonClick('#tool_PEN3')) {
          svgCanvas.setStrokeWidth(20);
          svgCanvas.setOpacity(0.3);
          $('#stroke_width').val(20).trigger("change");
          $('#group_opacity').val(30).trigger("change");
/*
          svgCanvas.setStrokeWidth(40);
          svgCanvas.setOpacity(0.4);
*/
          var picker = "stroke";
          var color = '#FFFF33';
          var paint = null;
          ...
          $('#tool_PEN1 div').removeClass("qtype_drawing_active_selection");
          $('#tool_PEN2 div').removeClass("qtype_drawing_active_selection");
          $('#tool_PEN3 div').addClass("qtype_drawing_active_selection");
          $('#tool_PEN4 div').removeClass("qtype_drawing_active_selection");
      //  }
      };
      var quicksizechange = function(e) {
          ...
          if(penortext == 'pen') {
            ...
            $('#stroke_width').val(0); // Stroke should not have an inital effect.
            $('#font_size').val(size);
      ...
      var clickquickcolor = function(e) {
          ...
          Editor.paintBox[picker].setPaint(paint);
          svgCanvas.setColor(picker, hexcolor, noUndo);
      ...
      var clickLine = function() {
        if (toolButtonClick('#tool_line')) {
          ...
          $("#path_node_panel").hide();
          $("#selected_panel").hide();
          $("#multiselected_panel").attr("style", "display: none !important");
        }
      };
      ...
      var clickRect = function(){
        ...
        if (toolButtonClick('#tool_rect')) {
          ...
          svgCanvas.setMode('rect');updateContextPanel();
          resetSelectedPresetsCSS();
        }
      };
      ...
      var clickFullscreen = function(){
        if (toolButtonClick('#tool_fullscreen')) {
      ...
      var dblclickZoom = function(){
        if (toolButtonClick('#tool_zoom')) {
          zoomImage(0.5); // 0.5 Added by Amr to zoom out on double click
          setSelectMode();
          resetSelectedPresetsCSS();
        }
      };

      var clickText = function(){console.error("text pressed...");
        if (toolButtonClick('#tool_text')) {
          /* amr 2019
           // Amr - load panels for the pen before clicking
           svgCanvas.clearSelection(true);
           updateContextPanel();
           $("#canvas_panel").hide();
           $("#drawingpreset_panel").hide();
           ...
           $("#stroke_panel").show();
           $('#selected_panel').show();
           $("#textpreset_panel").show();
           var urlparameters = getUrlParameters();
           if(urlparameters.reduced == "1") {
             $('#strokewidth_div').hide();
             $('#tool_opacity').hide();
             $('#advanced_drawtext_div').hide();
           }
           */
           // LMDL-186
           resetSelectedPresetsCSS();
           $('#font_size').val(24);
           svgCanvas.resetFontSize(24);
           $('#stroke_width').val(1);
           svgCanvas.resetStrokeWidth(1);
      ...
      var UNSelecElement = function() {
        if (selectedElement != null || multiselected) {
          alert('un selectedElement');
          //svgCanvas.cutSelectedElements();
        }
      };
      ...
      var clickEnterFullscreen = function(){
    		questionID = getCurrentMoodleQuestionID();
    		var theiframeid = window.parent.document.getElementById('qtype_drawing_editor_'+questionID);

    		theiframeid.style.position = "fixed";
    		window.parent.document.getElementById('qtype_drawing_drawingwrapper_'+questionID).style.zIndex = "999999999999999";
    		theiframeid.style.left = "0px";
    		theiframeid.style.top = "0px";

    		theiframeid.style.width = "100%";
    		theiframeid.style.height = "100%";
    		theiframeid.style.overflow = "hidden";
    		theiframeid.style.height = "100%";

    		$("#tool_exitfullscreen").show();
    		$("#tool_enterfullscreen").hide();
    		updateCanvas(true);
      ...
      var clickExitFullscreen = function(){
    		...
    		theiframeid.style.position = "";
    		window.parent.document.getElementById('qtype_drawing_drawingwrapper_'+questionID).style.zIndex = "";
    		theiframeid.style.left = "";
    		theiframeid.style.top = "";

    		theiframeid.style.width = "";
    		theiframeid.style.height = "";
    		theiframeid.style.overflow = "";
    		theiframeid.style.height = "";

    		$("#tool_exitfullscreen").hide();
    		$("#tool_enterfullscreen").show();
      };
      ...
      function getUrlParameters(){
        ...
        if (location.search) {
            ...
            for (var i = 0; i < parts.length; i++) {
                ...
                if (!nv[0]) continue;
                urlparameters[nv[0]] = nv[1] || true;
            }
        }
        return urlparameters;
      }
      function setDemoBackground(){
          // Now you can get the parameters you want like so:
          var urlparameters = getUrlParameters();
          var whichbackground = urlparameters.bg;
          switch (whichbackground){
            case "1":
              imgURL = 'images/q/bg1.png';
              break;
            case "2":
              imgURL = 'images/q/bg2.png';
              break;
            case "3":
              imgURL = 'images/q/bg3.png';
              break;
            case "4":
              imgURL = 'images/q/bg4.png';
              break;
            case "5":
              imgURL = 'images/q/bg5.png';
              break;
              case "undefined":
                imgURL = '';
                break;
        ...
        if (imgURL != ''){
          //var imgURL ='images/bg1.png';
          var image = new Image();
          image.src = imgURL;
          image.onload = function () {
            svgCanvas.clear();
            updateCanvas();
            svgCanvas.setBackground("",imgURL);
            svgCanvas.setResolution(image.width, image.height);
          }
        }
        // end Amr background
      }
    ...
    //  }
      ...
      var Actions = function() {
        ...
        var tool_buttons = [
          ...
          // Preset Sizes - Amr
          {sel:'.qtype_drawing_size_pen', fn: quicksizechange, evt: 'click', key: ['P1', true]},
          // Preset Colors - Amr
          {sel:'.qtype_drawing_color_divsquare', fn: clickquickcolor, evt: 'click', key: ['P1', true]},
          ...
          // Added by Amr
          {sel:'#tool_fullscreen', fn: clickFullscreen, evt: 'mouseup', key: [modKey + 'F', true]},
          // {sel:'#tool_zoom_out', fn: clickZoomOut, evt: 'mouseup', key: ['Z-', true]},
          // End Added by Amr
          {sel:'#tool_clear', fn: clickClear, evt: 'mouseup', key: [modKey + 'N', true]},
		      {sel:'#tool_enterfullscreen', fn: clickEnterFullscreen, evt: 'mouseup', key: [modKey + 'F', true]},
		      {sel:'#tool_exitfullscreen', fn: clickExitFullscreen, evt: 'mouseup', key: [modKey + 'E', true]},
          ...
          {key: 'MX', fn: function(){setMathJaxMode()}},
      ...
      var updateCanvas = Editor.updateCanvas = function(center, new_ctr) {
        ...
        w = Math.min(w_orig, svgCanvas.contentW * zoom * multi); // was max.
        h = Math.min(h_orig, svgCanvas.contentH * zoom * multi); // was max.
...
//      });
  ...
  Editor.SaveDrawingToMoodle = function() {
    console.error("saving done..");
	  Editor.ready(function() {
			// save for moodle
			questionID = getCurrentMoodleQuestionID();
			CanvdrawingValue = getCurrentDrawingSVG();
			 if(window.parent.document.getElementById('pre_existing_background_data')) { // first time question creation
				  window.parent.$('#id_qtype_drawing_textarea_id_0').val(CanvdrawingValue);
				 // hidden input in edit mode
				  window.parent.document.getElementById('pre_existing_background_data').value = CanvdrawingValue;
			} else {
				if( questionID == 0) { // New question, different ID
					window.parent.$('#id_qtype_drawing_textarea_id_'+questionID).val(CanvdrawingValue);
				} else {
					window.parent.$('#qtype_drawing_textarea_id_'+questionID).val(CanvdrawingValue);

				}
			}
			 // Trigger moodle quiz autosave :-)
			window.parent.$('#qtype_drawing_drawingevent').val(Math.random().toString(36).substring(7));
	  });
