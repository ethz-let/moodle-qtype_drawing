...
class qtype_drawing extends question_type {
    public function extra_question_fields() {
        return array('qtype_drawing','drawingmode','backgrounduploaded','backgroundwidth','backgroundheight','preservear','drawingoptions');
        //
    }
    ...
    function export_to_xml($question, qformat_xml $format, $extra=null) {
      ...
      $expout .= "        <imagetype>" . $bgImageArray[0] .  "</imagetype>\n";
    	$expout .= "    </bgimage>\n";
    	/*
      $expout .= '<drawingmode>' . $question->drawingmode . '</drawingmode>';
      $expout .= '<backgrounduploaded>' . $question->backgrounduploaded . '</backgrounduploaded>';
      $expout .= '<backgroundwidth>' . $question->backgroundwidth . '</backgroundwidth>';
      $expout .= '<backgroundheight>' . $question->backgroundheight . '</backgroundheight>';
      $expout .= '<preservear>' . $question->preservear . '</preservear>';
      $expout .= '<drawingoptions><![CDATA[' . $question->drawingoptions  . ']]></drawingoptions>';

    	$contextid = $question->contextid;
    	$fs = get_file_storage();
    	$files   = $fs->get_area_files($contextid, 'qtype_drawing', 'qtype_drawing_image_file', $question->id, 'id');
    	*/
